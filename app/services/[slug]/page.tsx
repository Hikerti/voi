...
function getLocalService(slug: string) {
  return SERVICES.find((service) => service.slug === slug);
}

export async function generateMetadata({ params }: ServicePageProps): Promise<Metadata> {
  const { slug } = await params;
  const service = (await getCmsServiceBySlug(slug)) || getLocalService(slug);
  if (!service) return {};
    ...
    <main className="vs-page-shell vs-page-shell--dark service-detail-page">
      ...
      <section className="seo-copy rich-content" aria-labelledby="service-includes-title">
        ...
        <div className="seo-copy__columns">
          <p>{service.summary}</p>
          <p>Фиксируем задачу, собираем структуру, дизайн, адаптивную версию и сценарии заявки.</p>
        </div>
      </section>
